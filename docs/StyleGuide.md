...
### `PageSectionLayout` + `SectionCard`

- 페이지 바깥 래퍼는 `PageSectionLayout`을 사용한다.
- 내부 섹션 카드는 `SectionCard`를 사용한다.
- 섹션 배치는 각 페이지에서 `grid`, `flex`, `space-y-*`로 직접 조합한다.

<img src="./attach/section_img.png" alt="섹션 레이아웃 예시" width="800" />
