...
3. [API 호출 패턴 (axios)](#3-api-호출-패턴-axios)
...
`vite.config.js`에 `/api` → `http://localhost:3000` 프록시가 설정되어 있다.

```js
// 이렇게 쓰면 → http://localhost:3000/transactions 로 요청됨
axios.get('/api/transactions')
```

절대 경로(`http://localhost:3000/...`)를 직접 쓰지 않는다.
...
// 페이지네이션: _page, _limit
axios.get('/api/transactions?_page=1&_limit=10')

// 필터: 필드명=값
axios.get('/api/transactions?type=expense')
axios.get('/api/transactions?type=expense&category=식비')
```
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 py-3 border-b border-border">
